...
const quizQuestions = [
    ...
    // === 5 MANATLIQ SUALLAR (41-50) (Variantssız / Yazılı Cavab) ===
    { q: "Kişilərin 30%-i keçəldir. Keçəllərin 50%-i, saçlıların 10%-i papaq taxır. Papaqlı birinin keçəl olma ehtimalı neçə faizdir? (Nümunə: 68.2%)", o: [], c: "68.2%", p: 5 },
    { q: "25 at var, 5 yollu trek var. Ən sürətli 3 atı tapmaq üçün saniyəölçənsiz minimum neçə yarış lazımdır?", o: [], c: "7", p: 5 },
    { q: "100 nəfərdən 10-u qırmızı, 90-ı qara papaqdadır. Hər kəs başqasını görür. Ən az 1 qırmızı var. Qırmızılar neçənci dəqiqədə çıxacaq?", o: [], c: "10", p: 5 },
    { q: "3 dost 30 manat ödəyir. Menecer 5 manat qaytarır, ofisiant 2-ni götürüb 3-ünü dostlara verir. 9x3=27 + 2 = 29 manat edir. 1 manat haradadır? (Qısa cavab yazın: məs. otel haqqıdır)", o: [], c: "otel haqqıdır", p: 5 },
    { q: "1-dən sonsuza qədər olan ədədlər çoxluğunda tək ədədlər çoxdur, yoxsa cüt ədədlər?", o: [], c: "Eynidir", p: 5 },
    { q: "Bərbər yalnız özü üzünü qırxmayanları qırxır. Bərbər öz üzünü qırxırmı?", o: [], c: "Paradoksdur", p: 5 },
    { q: "4 litrlik və 9 litrlik qablarla dəqiq neçə litr suyu ardıcıl fərqlərlə almaq olar?", o: [], c: "6", p: 5 },
    { q: "AZE=32 (A=1, Z=26, E=5). Ölkə kodları məntiqi ilə TUR neçə edir? (T=20, U=21, R=18)", o: [], c: "61", p: 5 },
    { q: "10 mərtəbəli binada 1-cidə 1 nəfər minir, hər mərtəbədə minənlərin sayısı mərtəbə nömrəsi qədərdir. 10-da neçə nəfər düşər?", o: [], c: "55", p: 5 },
    { q: "Dünyada indicə siz bu sualı oxuyanda təxminən neçə min nəfər sizinlə eyni anda gözünü qırpdı?", o: [], c: "500", p: 5 }
];
...
function startGame() {
    ...
    // Bütün 50 sualı oyunçu üçün tamamilə qarışdırırıq
...
function loadQuestion() {
    ...
    const currentData = randomizedQuestions[currentQuestionIndex];
    ...
    const textInput = document.getElementById("user-text-answer");
    
    optionsContainer.innerHTML = "";
    textInput.value = "";

    // 5 AZN-lik suallarda variantlar gizlənir, mətn sahəsi açılır
    if (currentData.p === 5) {
        ...
        document.getElementById("submit-text-answer-btn").onclick = () => {
            ...
            const correctAns = currentData.c.trim().toLowerCase();
            
            if (userAns === "") {
                alert("Zəhmət olmasa cavabınızı yazın!");
                return;
            }
            
            document.getElementById("submit-text-answer-btn").disabled = true;
            ...
            if (userAns === correctAns) {
                ...
                feedbackDiv.innerText = `Səhvdir! ❌ (Doğru cavab: ${currentData.c})`;
                feedbackDiv.className = "feedback wrong";
            }

            setTimeout(() => {
                document.getElementById("submit-text-answer-btn").disabled = false;
                currentQuestionIndex++;
                loadQuestion();
            }, 2000);
        };
    } else {
        // Digər suallarda normal variantlar göstərilir
